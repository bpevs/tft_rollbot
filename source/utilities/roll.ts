...
type RollResults = { [playerName: string]: RollResult };
...
export function roll(
  ...
  playerNames.forEach((name) => {
    ...
    const king: string = getRandom(
      Array.from(availableKings)
        .filter((king: string) => !takenKings.includes(king)),
    );
    ...
    availableTypes = availableTypes
      .filter((typeName: string) => (typeName !== type));

    availableOrigins = availableOrigins
      .filter((originName: string) => (originName !== origin));
